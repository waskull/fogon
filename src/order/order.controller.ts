...
export class OrderController {
    ...
    async create(@Body() dto: createOrderDto, @User() user: userEntity){
        ...
        if(!order.bought_by) throw new NotFoundException('Cliente Invalido');
        ...
        const res = await this.orderService.create(order, orderItems);
        res.forEach(async element => {
            let item = new Item();
            item = await this.itemService.getOneByItem(element.item.id);
    ...
    async delete(@Param('id', ParseIntPipe) id: number, @Res({ passthrough: true }) res){
        ...
        order.order_items.forEach(async element => {
            let item = new Item();
            item = await this.itemService.getOneByItem(element.item.id);
            if (item.stock-element.quantity <= 0) item.stock = 0;
            else item.stock -= element.quantity;
            this.itemService.reduceInventory(item);
        });
