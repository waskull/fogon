...
export class OrderService {
    ...
    async create(order: Order, newItems: OrderItems[]): Promise<OrderItems[]> {
        ...
        newItems.forEach(async e => {
            ...
            // let inventory = await this.itemRepository.findOne({ where: {  id: e.item.id }  });
            // inventory.stock += e.quantity;
            
            // const saveInventory = await this.itemRepository.save(inventory);
        });

        // let inventoryy = await this.itemRepository.findBy({id:In(newItems.map(i => i.id))});
